...
object PathSegment {
  ...
  def add(paths: List[PathSegment], newPath: List[String], entry: DiffEntry): List[PathSegment] = {
    ...
    res.sortWith( (a,b) => {
      val aFolder = if (a.children.size > 0) 0 else 1
      val bFolder = if (b.children.size > 0) 0 else 1
      val foldDiff = aFolder - bFolder
      if (foldDiff < 0) {
        true
      } else if (foldDiff > 0) {
        false
...
object ChangeSummary {
  def fromDiffs(diffs: Iterable[DiffEntry]): ChangeSummary = {
    ...
    val trees = diffsAndPaths.foldLeft(start) {
      case (segments, (diff, path)) =>
